...
#normal kernel and normal-inverseWish base
#H0 and H are the number of mixture components used in the approximation 
#of the infinite mixtures
telescopic_HDP_NNIW_multi_3L1P <- function(data, colayer, hyper=c(1, 1, 0.1),
  ...
  kernel_eval_joint = function(x, mean, sigma){
    if(length(x)==0){return(0)}else{return(sum(dmvnorm(x, mean, sigma, log = TRUE)))}
  }
  ...
  kernel_eval = function(x, mean, sigma){
    ...
    for (jj in (1:dim(mean)[1])){
      temp = c(temp, dmvnorm(x, mean[jj,], sigma[jj,,], log = TRUE))
    }
    return(temp)
  }
  ...
  posterior_sampler = function(x, mu0_prior = NULL, k0 = 0.1, Lambda0 = NULL, v0 = NULL){
    ...
    #Sigma_post = rinvwishart(vn, (Lambdan))
