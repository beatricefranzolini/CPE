# Simulation scenario A with two layers 
# The following code simulate data (scenario A) 
...
# Figure S5.1(panel A), Figure S5.2, Figure S6.1(panel A), Figure S8.1, 
# First row Table S8.1
...
#estimate telescopic ###########################################################
print("SIMULATION N.A - t-HDP")
if(run_MCMC){
totiter = 100000; burnin = 50000
set.seed(1)
m_saved =  telescopic_HDP_NN_uni(data, totiter = totiter)
write.table(m_saved[,1,], file = paste("02A_layer_1.csv",sep=""), row.names = FALSE,
            col.names= FALSE)
write.table(m_saved[,2,], file = paste("02A_layer_2.csv",sep=""), row.names = FALSE,
            col.names= FALSE)
}else{
  L = 2
  n_tot = 200
  ...
  print("MCMC")
  for(l in 1:L){
    temp = as.matrix(read.table(paste("output/02A_layer_",l,".csv", sep="")))
    m_saved[,l,] = temp
  }
}
...
p2<- insert_yaxis_grob(p1, ydens, grid::unit(.2, "null"), position = "right")
print(ggdraw(p2))
print("Figure S5.2 - panel C (same as Figure S6.1 - panel A) just printed")
...
print("Figure S8.1 - panel A just printed")


#effectiveSize(ri)/totiter
print("Simulation study n.A, Table S8.1 Layer 1:")
...
title(main="Rand Index layer 2 - Trace plot \n Conditional algorithm",
      cex.lab=0.75)
abline(v = burnin, col="red", lwd=3, lty=2)
print("Figure S8.1 - panel B just printed")

# effectiveSize(ri)/totiter
print("Simulation study n.A, Table S8.1 Layer 2:")
print(effectiveSize(ri[(totiter-burnin+1):totiter])/(totiter-burnin))

#estimate independent layers ##################################
print("SIMULATION N.A - Independent")
if(run_MCMC){
  ...
  write.table(c_layer1, file = paste("04A_layer_1.csv",sep=""), row.names = FALSE,
              col.names= FALSE)
  write.table(c_layer2, file = paste("04A_layer_2.csv",sep=""), row.names = FALSE,
              col.names= FALSE)
}else{
  L = 2
  n_tot = 200
  totiter = 1000; burnin = 500
  c_layer1 = as.matrix(read.table(paste("output/04A_layer_1.csv", sep="")))
  c_layer2 = as.matrix(read.table(paste("output/04A_layer_2.csv", sep="")))
}
...
p2 <- insert_yaxis_grob(p1, ydens, grid::unit(.2, "null"), position = "right")
print(ggdraw(p2))
print("Figure S5.2 - panel A just printed")

#estimate constant layers#############################################################
print("SIMULATION N.A - Constant")
if(run_MCMC){
  group1 = c(rep(1, 200)); totiter = 1000; burnin = 500
  set.seed(1)
  c_layer_constant = HDP_normal_normal(data, group1, totiter = totiter)
  write.table(c_layer_constant, file = paste("05A_layer_1and2.csv",sep=""), row.names = FALSE,
              col.names= FALSE)
}else{
  L = 2
  n_tot = 200
  totiter = 1000; burnin = 500
  c_layer_constant = as.matrix(read.table(paste("output/05A_layer_1and2.csv", sep="")))
}

psm = psm(c_layer_constant[(totiter - burnin + 1):totiter,])

VIminlayer = minVI(psm, c_layer_constant[(totiter - burnin + 1):totiter,], 
                   method = "all")$cl[1,]   
VIminlayer1 = VIminlayer
VIminlayer2 = VIminlayer
# write.table(VIminlayer1, file = paste("layer_constant_pointEst.csv",sep=""), 
#             row.names = FALSE,
#             col.names= FALSE)
...
p2 <- insert_yaxis_grob(p1, ydens, grid::unit(.2, "null"), position = "right")
print(ggdraw(p2))
print("Figure S5.2 - panel B just printed")
