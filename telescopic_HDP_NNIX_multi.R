#t-HDP _ conditional algorithm - NNIX model - multivariate
library(progress)#to draw the progress bar
library(LaplacesDemon) #for scale chi squared 
...
#hyper are the 2 concentration parameters, the scale parameter k0 for the mean and
#the scale paramater sigma^2_0 for the variance
...
telescopic_HDP_NNIX_multi <- function(data, colayer, hyper=c(0.1, 0.1, 0.1),
                                      alpharandom = FALSE,
  ...
  kernel_eval_joint = function(x, mean, sigma){
    return(sum(dnorm(x, mean, sqrt(diag(sigma)), log = TRUE)))
  }
  ...
  kernel_eval = function(x, mean, sigma){
    ...
    for (jj in (1:dim(mean)[1])){
      sigma_temp = diag(sigma[jj,,])
      temp = c(temp, sum(dnorm(x, mean[jj,], sqrt(sigma_temp), log = TRUE)))
    }
    return(temp)
  }
  
  posterior_sampler = function(x, mu0_prior = NULL, k0 = 0.1, Lambda0 = NULL, v0 = NULL){
    ...
    Sigma_post = diag(rinvchisq(p, df = vn - p + 1, diag(Lambdan) / (vn - p + 1)))
    mu_post = rnorm(p, mun, sqrt(kn^(-1)*diag(Sigma_post)))
