my_prior_LSBP <- function(p_kernel, p_mixing, b_kernel = rep(0, p_kernel), B_kernel = diag(10^6, p_kernel),
                       b_mixing = rep(0, p_mixing), B_mixing = diag(1, p_mixing), a_tau = 0.1, b_tau = 0.1) {
  if (missing(p_kernel) | missing(p_mixing)) {
    stop("Please provide a value for p_kernel and p_mixing")
  }
  prior <- list(
    b_kernel = b_kernel, B_kernel = B_kernel, b_mixing = b_mixing, B_mixing = B_mixing,
...
my_LSBP_Gibbs_univ <- function(y, X, prior, H, R, burn_in, method_init, verbose, verbose_step) {
  # Fixed quantities
  n <- length(y)
  p <- NCOL(X)
  ...
  list(param = list(beta_mixing = beta_mixing_out, beta_kernel = mu_out, tau = tau_out, nclust = nclust_out, G = G_out), logposterior = logpost)
}

my_LSBP_Gibbs_multi <- function(y, X1, X2, H, R, prior, burn_in, method_init, verbose, verbose_step) {
  # Fixed quantities
  n <- length(y)
  p_kernel <- NCOL(X1)
  p_mixing <- NCOL(X2)
  ...
  list(param = list(beta_mixing = beta_mixing_out, beta_kernel = beta_kernel_out, tau = tau_out, nclust = nclust_out, G = G_out), logposterior = logpost)
}
...
my_LSBP_Gibbs <- function(Formula, data, H, prior, control = control_Gibbs(), verbose = TRUE) {
  ...
  if (missing(prior)) {
    prior <- my_prior_LSBP(p_kernel, p_mixing)
  }
  ...
  if (NCOL(X1) > 1) {
    out <- my_LSBP_Gibbs_multi(
      y = y, X1 = X1, X2 = X2, H = H, prior = prior, R = control$R, burn_in = control$burn_in,
      method_init = control$method_init, verbose = verbose, verbose_step = verbose_step
    )
  } else {
    out <- my_LSBP_Gibbs_univ(
      y = y, X = X2, H = H, prior = prior, R = control$R, burn_in = control$burn_in,
      method_init = control$method_init, verbose = verbose, verbose_step = verbose_step
    )
  }
