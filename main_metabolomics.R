# The following estimate the t-HDP on the real data
# results are presented in Section 7.2 of the paper and 
# Section S7 of the Supplement

#Simulated data, telescopic model with DP at first layer 
#Section S6 of the supplement

#WARNING 1 : REAL DATA ARE NOT AVAILABLE
...
library(purrr)   # version 1.0.2                  for k-means exploratory
library(cluster) # version 2.1.4                  for k-means exploratory
library(rstatix) # version 0.7.2                  for t-test exploratory
...
library(pheatmap) # version 1.0.12        to plot
...
library(CholWishart)   # version 1.1.4
...
#(no data available to the public), 
#if false the code upload the outcome of the mcmc from a csv
run_MCMC = FALSE 


#data are proprietary and not available to the public 
#read data (WARNING: DATA FILE NOT PROVIDED)
#'not to run 
# BMI_rawdata = read.table("metabolomics/data/BMI_zscores_20200608.csv", 
# sep=",", header = TRUE)
...
#l=1
layer1_chain = read.csv("output/02R_layer_1.csv", sep =" ", header = FALSE)
...
#l=2
layer2_chain = read.csv("output/02R_layer_2.csv", sep =" ", header = FALSE)
...
#l=3
layer3_chain = read.csv("output/02R_layer_3.csv", sep =" ", header = FALSE)
...
# names_vector <- c("Clinical LDL Cholesterol", "HDL Cholesterol", "Triglycerides",
#                   "Phosphoglycerides", "Cholines Phosphoglycerides", 
#                   "Sphingomyelins", "APO A1", "APO B", "Omega 3", 
#                   "Omega 6", "Poly-Unsaturated FA (PUFA)", 
#                   "Mono-Unsaturated FA (MUFA)", "Saturated FA (SFA)", 
#                   "Linoleic acid", "Docosahexaenoic acid (DHA)", 
#                   "Alanine", "Glutamine", "Glycine", "Histidine", 
#                   "Isoleucine", "Leucine", "Valine", "Phenylalanine", 
#                   "Tyrosine", "Glucose", "Lactate", "Pyruvate",
#                   "Citrate", "beta-Hydroxybutyric acid (bOHbutyrate)", 
#                   "Acetate", "Acetoacetate", "Acetone", "Creatinine", 
...
# tab_kruskal = NULL
# for (met in 1:35){
#   tab_kruskal = c(tab_kruskal, 
#             kruskal.test(sum_met_clust[,met] ~ sum_met_clust$cluster)$p.value)
# }
# 
